...
{
    half NdotL = saturate(dot(normal, lightDir) * 0.5 + 0.5) * 10;
...
half3 LightingSpecular(half3 lightColor, half3 lightDir, half3 normal, half3 viewDir, half4 specular, half smoothness)
{
    ...
    half NdotH = half(saturate(dot(normal, halfVec) * 0.5 + 0.5)) * 10;
...
{
    half NdotL = saturate(dot(normalWS, lightDirectionWS) * 0.5 + 0.5) * 10;
